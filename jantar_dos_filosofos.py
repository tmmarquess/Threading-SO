    ...
    def philosopher(self, i):
        ...
        while self.meals[i] > 0:
            ...
            if self.chopsticks[i].acquire(
                timeout=1
            ):  # check if the chopstick is been used for someone
                self.chopstick_holders[
                    i
                ] = " /   "  # if chopstick is not been used, set that to the philosopher
                time.sleep(
                    random.random()
                )  # after get a chopstick, put the philosopher to sleep
                if self.chopsticks[j].acquire(
                    timeout=1
                ):  # here we will check if the next chopstick is been used
                    self.chopstick_holders[
                        i
                    ] = " / \\ "  # if chopstick is not been used, set that to the philosopher
                    self.status[
                        i
                    ] = "  C  "  # set de status to: C (comendo) for the philosopher
                    time.sleep(random.random())  # put the philosopher to sleep
                    self.meals[
                        i
                    ] -= 1  # subtract one meal of the philosopher that are eating
                    self.chopsticks[
                        j
                    ].release()  # after eat, release the second chopstick
                    self.chopstick_holders[
                        i
                    ] = " /   "  # set just one chopstick to the philosopher
                self.chopsticks[i].release()  # release the first chopstick
                self.chopstick_holders[
                    i
                ] = "     "  # set none chopstick to the philosopher
...
def main():
    ...
    while ( sum(dining_philosophers.meals) > 0):  # will be checking if still have meals to be eaten
        ...
        )  # print the status that the philosopher is
        ...
        )  # print the quantity of meals of each philosopher
